...
1. 订单金额必须由服务端重新计算，不能信任客户端传入金额。
2. 创建订单后需要校验订单状态、总金额与库存扣减，接口返回成功不代表数据一定正确。
3. 库存不足与重复提交订单是交易链路的高风险异常场景。
4. 测试代码按“接口封装 / 测试数据 / 用例 / 数据校验”分层，便于维护和扩展。

## GitHub Actions

推送到 GitHub 后，`.github/workflows/tests.yml` 会在 Python 3.10 和 3.11 环境执行测试，并上传 JUnit 与 Allure 原始结果作为构建产物。
